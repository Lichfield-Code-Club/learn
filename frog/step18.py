...
initial_logs = 12     # Start with more logs for easier beginning
...
def create_logs():
    ...
    # Ensure the last section has logs closer to the frog's starting position
    bottom_section_height = 120  # Height of the bottom section where we want closer logs
    main_section_height = water_height - bottom_section_height
    
    # Calculate number of logs for each section
    bottom_logs = max(3, num_logs // 4)  # At least 3 logs in bottom section
    main_logs = num_logs - bottom_logs
    ...
    for i in range(num_logs):
        ...
        rect.y = WATER_START + gap + (i * (log_height + gap))
